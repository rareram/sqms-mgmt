...
- **동적 모듈 시스템**: 플러그인 방식으로 확장 가능
  1. **LDAP 관리**: 퇴사자 관리, 사용자 검색 및 LDAP 설정
    - OpenLDAP/Active Directory 지원
    - 퇴사자 관리 및 계정 상태 확인
    - 사원 구분별 필터링 (정직원/협력사/기타)
    - 사용자 검색 및 CSV 내보내기
  2. **GitLab 관리**: 저장소 관리, 사용자 관리, 미사용 저장소 조회
    - 저장소 관리 및 용량 모니터링
    - 미사용 저장소 탐지
    - 사용자/팀 관리
    - 저장소 통계 및 시각화
  3. **Redmine 관리**: 프로젝트 관리, 사용자 관리
    - 프로젝트/사용자 관리
    - 퇴사자 계정 매칭 및 권한 확인
    - 비활성 프로젝트 탐지
    - CSV 업로드를 통한 일괄 처리
  4. **Grafana 관리**: 팀 관리, 폴더 권한 관리
    - 팀 관리 및 폴더 권한 관리
    - Dashboard Descriptino 편집기
    - SSL 우회 옵션 지원
    - 권한 현황 시각화
